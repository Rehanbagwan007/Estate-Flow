...
-- Allow admins and super_admins to read the parameters
CREATE POLICY "Allow read access to admins and employees"
...
USING (
  ...
  (get_my_claim('user_role'::text)) = '"sales_manager"'::jsonb OR
  (get_my_claim('user_role'::text)) = '"sales_executive_1"'::jsonb OR
  (get_my_claim('user_role'::text)) = '"sales_executive_2"'::jsonb
);

-- Allow admins and super_admins to update the parameters
...
INSERT INTO public.salary_parameters (parameter_name, rate, description)
VALUES
    ('per_call_rate', 10, 'Amount paid per completed call'),
    ('per_meeting_rate', 50, 'Amount paid per completed meeting/appointment'),
    ('per_km_travel_rate', 5, 'Amount paid per kilometer traveled for site visits')
